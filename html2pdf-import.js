...
class Html2Pdf {
    ...
    cloneNode () {
        ...
        Array.from(document.querySelectorAll(this.classes)).forEach((dom, index) => {
            html2pdf.appendChild(dom)
        })
        document.body.append(html2pdf)
        return this
    }
    // 绘制html
    drawHtmlCanvas () {
        let doms = Array.from(document.querySelectorAll(`#html2pdf > ${this.classes}`)).map(dom => html2canvas(dom, { scale: this.opt.scale }))
...
let htmlpdf = (classes, opt) => {
    return new Html2Pdf(classes, opt)
}
